...
//
// Baked-in extraction gotchas (confirmed by grepping src/AICodingServices.MSBuild/MSBuildWorkspaceLoader.cs):
...
public sealed class ClaudeSmokesAuthoringWinFormsTests
{
    ...
    public async Task ClaudeSmokes_authoring_order_graph_over_winforms_copy_indexes_new_graph_without_dropping_customer()
    {
        ...
        string tempRoot = Path.Combine(
            Path.GetTempPath(), "AICodingServicesClaudeSmokesAuthoringWinForms", Guid.NewGuid().ToString("N"));
        ...
        string databasePath = Path.Combine(
            Path.GetTempPath(), "AICodingServicesClaudeSmokesAuthoringWinForms", Guid.NewGuid().ToString("N"), "index.sqlite");
    ...
    private static string FindRepositoryRoot()
    {
        ...
        {
            if (File.Exists(Path.Combine(directory.FullName, "AICodingServices.slnx")))
            {
        ...
        throw new DirectoryNotFoundException("Could not find AICodingServices.slnx.");
    }
}
