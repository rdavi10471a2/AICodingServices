...
public sealed class McpVsGrepTokenBenchmarkTests
{
    ...
    private const string DefaultOutputDir = @"C:\VSCodeProjects\AICodingServices\runtime\token-benchmark";
    ...
    public async Task FullCoverage_mcp_vs_grep_token_baseline()
    {
        ...
        string tempRoot = Path.Combine(Path.GetTempPath(), "AICodingServicesTokenBench", Guid.NewGuid().ToString("N"));
    ...
    // Mirrors AICodingServices.McpServer's AICodingServicesIndexedReferenceResult (the default "lean" find_indexed_references shape).
