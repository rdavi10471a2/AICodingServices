...
get_solution_index_tree(skipFiles: 0, maxFiles: 500)
...
`get_solution_index_tree` is the read-only project/folder/file orientation surface for planning. If `hasMore` is true, continue with `skipFiles: nextSkipFiles` rather than assuming the first response is complete.
...
- Planning starts in the CodexUI task board when it is available. Use `/workflow-board` to keep exactly one Active task, task notes, and intended solution-relative files visible before watched-source mutations.
- The Active task is the human-readable planning authority. `start_monitor_session(filesPlanned: ...)` must match the Active task's intended files or a narrower subset explicitly agreed for the current edit.
- Until MCP task-context tools are exposed, do not invent an Active task from chat memory. If the task board state is unclear, ask the operator to select or create the Active task before mutating watched source.
